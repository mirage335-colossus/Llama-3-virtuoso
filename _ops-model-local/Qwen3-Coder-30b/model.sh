...
_model-Qwen3-Coder-30b() {
    ...
    local current_fileDir='Qwen3-Coder-30b'
    ...
    if _if_cygwin
    then
        _messagePlain_nominal '(cygwin)' ollama pull qwen3-coder:30b
        ...
        [[ -e "$current_user_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/30b ]] && current_origModelInstalled='true' && echo current_origModelInstalled
        ...
        ollama ls qwen3-coder:30b
        
        _messagePlain_probe 'while... ollama pull'
        local currentIteration=0
        #while ( [[ ! -e "$current_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/30b ]] || [[ ! -e "$current_OLLAMA_MODELS"/get_qwen3coder ]] ) && ( ! env OLLAMA_MODELS="$current_OLLAMA_MODELS_msw" ollama pull qwen3-coder:30b || ! env OLLAMA_MODELS="$current_OLLAMA_MODELS_msw" ollama ls | grep -i 'qwen3-coder:30b' )
        while ( [[ ! -e "$current_user_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/30b ]] || [[ ! -e "$current_user_OLLAMA_MODELS"/get_qwen3coder ]] ) && ( ! ollama pull qwen3-coder:30b || ! ollama ls | grep -i 'qwen3-coder:30b' )
        do
            ...
            then
                _messagePlain_bad 'bad: ''ollama pull qwen3-coder:30b'
                _messageFAIL
                #return 1
            fi
        ...
        [[ -e "$current_user_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/30b ]] && mkdir -p "$current_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder && rsync --size-only -r -v "$current_user_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/. "$current_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/.
        [[ ! -e "$current_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/30b ]] && _messageFAIL

        local currentBlobHash
        grep -aoE 'sha256:[0-9a-f]{64}' "$current_user_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/30b | tr ':' '-' | while IFS= read -r currentBlobHash; do
        ...
        ! ollama ls | grep -i 'qwen3-coder:30b' && _messageFAIL
    fi
    ...
    if ! _if_cygwin
    then
        _messagePlain_nominal ollama pull qwen3-coder:30b
        ...
        _messagePlain_probe 'while... ollama pull'
        local currentIteration=0
        #while ( [[ ! -e "$current_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/30b ]] || [[ ! -e "$current_OLLAMA_MODELS"/get_qwen3coder ]] ) && ( ! env OLLAMA_MODELS="$current_OLLAMA_MODELS" ollama pull qwen3-coder:30b || ! env OLLAMA_MODELS="$current_OLLAMA_MODELS" ollama ls | grep -i 'qwen3-coder:30b' )
        while ( [[ ! -e "$current_user_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/30b ]] || [[ ! -e "$current_user_OLLAMA_MODELS"/get_qwen3coder ]] ) && ( ! ollama pull qwen3-coder:30b || ! ollama ls | grep -i 'qwen3-coder:30b' )
        do
            _messagePlain_probe 'iteration'
            (( currentIteration++ ))
    ...
    _messagePlain_nominal 'ollama create Qwen3-Coder-30b-virtuoso'
    ...
    (
        ...
        ollama create Qwen3-Coder-30b-virtuoso -f Modelfile && echo > "$current_user_OLLAMA_MODELS"/get_qwen3coder
        currentExitStatus="$?"

        #echo "FROM Qwen3-Coder-30b-virtuoso:latest" > Modelfile-32k
        ...
        #ollama create Qwen3-Coder-30b-32k-virtuoso -f Modelfile-32k
    ...
    [[ "$current_origModelInstalled" != "true" ]] && ollama rm qwen3-coder:30b
    #rm -f "$current_user_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/qwen3-coder/30b
}


_experiment-Qwen3-Coder-30b() {
    _stopwatch curl -X POST http://localhost:11434/api/chat \
    -H "Content-Type: application/json" \
    -d '{
        "model": "Qwen3-Coder-30b-virtuoso",
        "messages": [
