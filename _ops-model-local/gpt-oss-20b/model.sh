...
_model-gpt-oss-20b() {
    ...
    local current_fileDir='gpt-oss-20b'
    ...
    if _if_cygwin
    then
        _messagePlain_nominal '(cygwin)' ollama pull gpt-oss:20b
        ...
        do
            ...
            then
                _messagePlain_bad 'bad: ''ollama pull gpt-oss:20b'
                _messageFAIL
                #return 1
            fi
        ...
        ! ollama ls | grep -i 'gpt-oss:20b' && _messageFAIL
    fi
    ...
    if ! _if_cygwin
    then
        _messagePlain_nominal ollama pull gpt-oss:20b
    ...
    _messagePlain_nominal 'ollama create gpt-oss-20b-virtuoso'
    ...
    (
        ...
        ollama create gpt-oss-20b-virtuoso -f Modelfile && echo > "$current_OLLAMA_MODELS"/get_gpt-oss-20b
        currentExitStatus="$?"

        echo "FROM gpt-oss-20b-virtuoso:latest" > Modelfile-128k
        ...
        ollama create gpt-oss-20b-128k-virtuoso -f Modelfile-128k

        [[ "$?" == "0" ]] && [[ "$currentExitStatus" == "0" ]]
    )


    [[ "$current_origModelInstalled" != "true" ]] && ollama rm gpt-oss:20b
    #rm -f "$current_user_OLLAMA_MODELS"/manifests/registry.ollama.ai/library/gpt-oss/20b
}


_experiment-gpt-oss-20b() {
    _stopwatch curl -X POST http://localhost:11434/api/chat \
    -H "Content-Type: application/json" \
    -d '{
        "model": "gpt-oss-20b-virtuoso",
        "messages": [
